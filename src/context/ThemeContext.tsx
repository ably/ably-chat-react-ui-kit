...
export interface ThemeContextType {
   ...
   * @returns The system theme preference or null if not available
   */
  getSystemTheme: () => ThemeType | null;
...
export const ThemeProvider: React.FC<ThemeProviderProps> = ({
   ...
   */
  const getSystemTheme = useCallback((): ThemeType | null => {
  ...
  useEffect(() => {
    if (!detectSystemTheme || !supportsSystemTheme) return;
    ...
    const handleChange = (e: MediaQueryListEvent) => {
      ...
      if (persist && localStorage.getItem(THEME_STORAGE_KEY)) {
        return; // User has a saved preference, don't override
      }
  ...
  }, [detectSystemTheme, supportsSystemTheme, persist, setTheme]);
