...
vi.mock('@ably/chat/react', () => ({
  ChatRoomProvider: ({ children, name, options }: ChatRoomProviderProps) => (
    <div
      data-testid="chat-room-provider"
      data-room-name={name}
...
vi.mock('../../../components/molecules/room-list-item.tsx', () => ({
  RoomListItem: ({ roomName, isSelected, isCollapsed, onClick, onLeave }: RoomListItemProps) => (
    ...
    >
      <button data-testid={`select-room-${roomName}`} onClick={onClick}>
        Select {roomName}
      </button>
      <button data-testid={`leave-room-${roomName}`} onClick={onLeave}>
        Leave {roomName}
      </button>
    </div>
  ),
...
describe('RoomList', () => {
  ...
  it('renders a list of rooms', () => {
    render(<RoomList roomNames={roomNames} onSelect={mockOnSelect} onLeave={mockOnLeave} />);
  ...
  it('renders nothing when roomNames is empty', () => {
    const { container } = render(
      <RoomList roomNames={[]} onSelect={mockOnSelect} onLeave={mockOnLeave} />
    );
